<template>
  <div class="main">
    ...
    <div class="car__option__block" :style="{border: regionOptoin.id ? '1px solid green' : '2px solid red'}">
      <p>region</p>
      <select class="car__option__block" v-model="regionOptoin.id" @change="chooseRegion({id : regionOptoin.id })">
        {{ computedRegionSubregions }}
        <option v-for="region in regions" :value="region.id"> {{ region.name }} </option>
      </select>
    </div>

    <div class="car__option__block" v-if="regionOptoin.id" :style="{border: subregionOption.idOld  ? '1px solid green' : '2px solid red'}">
      <p>subregion</p>
      <select class="car__option__block" @change="chooseSubregion(subregionOption.id)" v-model="subregionOption.id">
        <option v-for="subregion in subregions[regionOptoin.subregions]" :value="subregion.id"> {{ subregion.id }}</option>
      </select>
    </div>
    ...
    <svg version="1.1" id="Belarus" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      ...
      <g id="regionsBorder">
        <polygon stroke="red" fill="white" stroke-width="3px" v-for="region in regions" :points="region.points"/>
      </g>

      <g id="cityes">
        <circle v-for="city in cityes" :id="city.id" :cx="city.cx" :cy="city.cy" :r="city.r"/>
      </g>

      <g id="subregions">
        <g v-for="subregion,index in subregions" :id="index">
          <polygon
            @mouseleave="leaveMap()"
            @mousemove="hoverMap($event, city.id)"
            @click="chooseSubregion(city.id)"
            v-for="city in subregion"
            :id="city.id"
            :points="city.points"/>
        </g>
      </g>
      ...
      <g id="regions">
        <polygon
          @mouseleave="leaveMap()"
          @mousemove="hoverMap($event, region.name)"
          @click="chooseRegion({id: region.id, name : region.name, subregions: region.subregions})"
  ...
  export default {
    data(){
      return {
        ...
        regions: [
          {
            ...
            subregions: "hrodnenskaiaVoblastsSubregions",
          ...
          },
          {
            ...
            subregions: "vitsebskaiaVoblastsSubregions",
          ...
          },
          {
            ...
            subregions: "mahiliouskaiaVoblastsSubregions",
          ...
          },
          {
            ...
            subregions: "homelskaiaVoblastsSubregions",
          ...
          },
          {
            ...
            subregions: "minskaiaVoblastsSubregions",
          ...
          },
          {
            ...
            subregions: "brestskaiaVoblastsSubregions",
        ...
        subregions: {
          hrodnenskaiaVoblastsSubregions: [
            {
              id: "hrodzenski-raion",
          ...
          mahiliouskaiaVoblastsSubregions: [
            {
              id: "horatski-raion",
    ...
    methods: {
      ...
      hoverMap(e,name){
        let title = this.$refs.tooltip;
        title.innerHTML = name;
        title.style.display = "block";
        title.style.left = e.layerX + 20 + 'px';
        title.style.top = e.layerY + 5 + 'px';
      },

      chooseRegion({id, name, subregions}){
        this.regionOptoin.idOld ? document.getElementById(this.regionOptoin.idOld).style.display = 'block' : '';
        this.subregionOption.idOld ? document.getElementById(this.subregionOption.idOld).style.opacity = '0.5' : '';
        this.subregionOption.idOld = '';
        this.regionOptoin.name = name;
        this.regionOptoin.subregions = subregions;
        this.regionOptoin.id = this.regionOptoin.idOld = id;
        document.getElementById(id).style.display = 'none';
      },

      chooseSubregion(id){
        this.subregionOption.idOld ? document.getElementById(this.subregionOption.idOld).style.opacity = '0.5' : '';
        this.subregionOption.idOld = this.subregionOption.id = id;
        document.getElementById(this.subregionOption.idOld).style.opacity = '1'
      },
    },
    computed: {
      computedRegionSubregions(){
        let item = this.regions.find(item => item.id === this.regionOptoin.idOld);
        item ? this.regionOptoin.subregions = item.subregions : '';
        this.$emit('carRegion', {region: this.regionOptoin.name, subregion : this.subregionOption.idOld})
      }
    }
  }
...
<style lang="scss" scoped>
  ...
  #hrodnenskaia-voblasts {fill: #79b2e0;}
  #vitsebskaia-voblasts {fill: #c1c4ed}
  #mahiliouskaia-voblasts {fill: #f0bfe9}
  #homelskaia-voblasts {fill: #cfe8aa}
  #minskaia-voblasts {fill: #dfd67c}
  #brestskaia-voblasts {fill: #d5c09c}
